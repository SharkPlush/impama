...
compile_app () {
  distrobox-create --name temp-compiling-impama --image debian --home /home/$USER/.cache/impama_temp_home_directory
  distrobox enter temp-compiling-impama -- bash -c "
  sudo apt update -y
  sudo apt upgrade -y
  sudo apt install golang git npm nodejs ruby ruby-dev gcc make curl rpm libiw-dev astyle cmake ninja-build libssl-dev python3-pytest python3-pytest-xdist unzip xsltproc doxygen graphviz python3-yaml valgrind -y
